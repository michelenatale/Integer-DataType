...
public readonly struct UInt256Ex : IUIntXEx<UInt256Ex>, IUInt256Ex<UInt256Ex>
{
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static UInt256Ex operator -(UInt256Ex value) => new UInt256Ex(NegateUnsigned(value.ToValues(), TypeSize / 8));

  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  private static UInt256Ex Negate(in UInt256Ex value)
  {
    var lohis = new ulong[] { 0 - value.LLO, 0 - value.LHI, 0 - value.RLO, 0 - value.RHI };
    if (value.LLO > 0) lohis[3]--;
    return new(lohis);
  }
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  private static uint[] Division(in uint[] left, in uint right, in int size, out uint remainder)
  {
    var r = 0UL;
    var result = new uint[size];
    for (var i = size - 1; i >= 0; i--)
    {
      var value = (r << 32) | left[i];
      result[i] = (uint)(value / right);
      r = value % right;
    }
    remainder = (uint)r;
    return result;
  ...
  private static uint[] LeftShift(in UInt256Ex value, in int shiftl, in int typesize)
  {
    ...
    return LeftShift(uints, shiftl, TypeSize / 4);
  }
  ...
  {
    var uints = value.ToUInts();
    return RightShift(uints, shiftl, TypeSize / 4);
  }
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static bool TryParse(ReadOnlySpan<char> value, out UInt256Ex uix)
  {
  ...
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static bool TryParse([NotNullWhen(true)] string value, out UInt256Ex uix)
  {
    uix = 0;
    ...
    for (int i = 0; i < str.Length; i++)
    {
      if ("0123456789".Contains(str[i]))
        uix = uix * 10 + ulong.Parse(str[i].ToString());
      else
      {
        uix = 0;
        return false;
